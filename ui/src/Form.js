...
export default class Form extends Component {
  ...
  render() {
    const answersFull = Object.keys(this.state.answers) >= Object.keys(this.state.questions || {})
              ...
              <h3>{this.state.title}</h3>
                <form>
                    {this.state.questions.map((question, i) => {
                        const maxAnswerLength = Math.max(...question.answers.map(s => s.length))
                                ...
                                <div className="col-11">
                                    {question.answers && Array.isArray(question.answers) ?
                                        question.answers.map((answer, j) => {
                                            ...
                                            const selectedStyle = selected ? {backgroundColor: '#6c757d', color: 'white'} : {}
                                            return (
                                                <div key={j}>
                                                    <button 
                                                        type="button" 
                                                        className="btn btn-outline-secondary mt-1 text-left"
                                                        onClick={() => this.setState({ answers: {...this.state.answers, [i]: j} })}
                                                        style={{...selectedStyle, width, overflowWrap: 'break-word'}}
                                                    >{answer}</button>
                                                </div>
                                        )})
